...
## Аналитика — интерактивные графики на вкладке «Статистика»

`/dashboard/stats` (`StatsTab.tsx`) — теперь полноценный дашборд с графиками на **recharts**, а не только карточки-цифры.

- **Backend**: `backend/routes/analytics.js`, `GET /api/admin/analytics?months=3|6|12` (по умолчанию 6). Один запрос считает всё разом за один проход по записям — без сложных SQL-агрегаций, для масштаба одного салона этого достаточно.
  - Помесячные ряды: `bookingsByMonth` (количество записей), `revenueByMonth` (выручка — только по записям со статусом `completed`, цена берётся из текущих цен `Service` по названию услуги — если цену услуги меняли задним числом, старые месяцы это не учитывают), `cancellationsByMonth`, `newClientsByMonth` (по `User.createdAt`), `utilizationByMonth` (загрузка мастера: `забронированные часы / доступные рабочие часы * 100`, доступные часы считаются как рабочие дни месяца × 10ч минус day_off/vacation/blocked_slot из `ScheduleBlock`).
  - `popularServices` — топ-8 услуг по числу записей за период (не по выручке).
  - `clients.{newInPeriod, returningInPeriod, repeatRate}` — «постоянный» клиент = 2+ незавершённых-неотменённых записи за всё время (не только в выбранном периоде); `repeatRate` — их доля среди клиентов, обратившихся в периоде.
  - `totals.{totalRevenue, totalBookings, totalCancellations, avgCheck}` — `avgCheck` = выручка / число `completed`-записей за период.
  - Модель "мастер" в проекте одна (соло-салон) — поэтому "загрузка мастеров" реализована как загрузка единственного мастера, а не разбивка по нескольким сотрудникам. Если появится несколько мастеров — понадобится поле `masterId` в `Appointment`/`ScheduleBlock` и разбивка метрики по каждому.
- **Frontend**: графики на `recharts` (добавлен в `package.json`) — бар-чарты (записи/новые клиенты/отмены), area-чарт (выручка), line-чарт (загрузка мастера, %), горизонтальный bar (популярные услуги), donut/pie (новые vs постоянные клиенты). Все графики тёмной темы, с общими стилями тултипов/цветов, обёрнуты в `ChartCard`. Селектор периода (3/6/12 месяцев) над графиками.
- Верхние карточки с мгновенными счётчиками (`Всего/Ожидают/Подтверждены/...`) остались как раньше — идут из `useAdminData` (не из аналитики), поэтому обновляются сразу при смене статуса записи, без похода за новой аналитикой.
