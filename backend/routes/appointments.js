...
async function sendTelegramNotification(appointment) {
  ...
  console.log('🟢 sendTelegramNotification() вызвана для записи', appointment.id)

  const token = (process.env.TELEGRAM_BOT_TOKEN || '').trim()
  const chatId = (process.env.TELEGRAM_CHAT_ID || '').trim()

  if (!token || !chatId) {
    console.log('⚠️  Telegram: токен или chat_id не настроен (см. backend/.env)')
    return
  }
  ...
  const text =
    ...
    (appointment.comment ? `💬 *Комментарий:* ${e(appointment.comment)}\n` : '') +
    `\n🆔 ID: \`${e(appointment.id)}\``

  console.log(`📲 Отправляю Telegram уведомление на chat_id: ${chatId}`)

  try {
    const data = await telegramRequest(token, { chat_id: Number(chatId), text, parse_mode: 'MarkdownV2' })
    ...
    const plain = text
      .replace(/\\([_*[\]()~`>#+\-=|{}.!])/g, '$1')
      .replace(/[*`]/g, '')
    const fallback = await telegramRequest(token, { chat_id: Number(chatId), text: plain })
    if (fallback.ok) {
      console.log('✅ Telegram уведомление отправлено (fallback без разметки)')
    } else {
      console.error('❌ Telegram fallback не сработал:', JSON.stringify(fallback))
    }
  } catch (err) {
    // Не блокируем создание записи, если Telegram недоступен
    console.error('❌ Telegram сетевая/системная ошибка:', err.code || err.message)
  }
}
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            reject(new Error(`Telegram вернул не-JSON ответ (status ${res.statusCode}): ${data.slice(0, 300)}`))
          }
        })
      }
    )
...
router.post('/', authMiddleware, async (req, res) => {
  console.log('🟢 [POST /api/appointments] получен запрос на создание записи, body:', req.body)
  ...
  if (!serviceList.length || !date || !time) {
    ...
    return res.status(400).json({ error: 'Укажите хотя бы одну услугу, дату и время' })
  }
  ...
  if (conflict) {
    console.log('🔴 Отклонено: пересекается с занятой записью —', date, time, 'duration:', duration)
...
router.get('/slots', async (req, res) => {
  ...
  const available = allSlots.filter(slot => {
    ...
    return !appointments.some(a => rangesOverlap(startHour, duration, timeToHour(a.time), a.duration || 1))
  })
