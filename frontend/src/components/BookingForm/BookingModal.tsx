...
export default function BookingModal({ onClose }: Props) {
  ...
  useEffect(() => {
    if (step === 2 && date) {
      ...
      apiFetch(`/api/appointments/slots?date=${date}&duration=${duration}`)
        .then(r => r.json())
  ...
  const book = async () => {
    ...
    try {
      const res = await apiFetch('/api/appointments', {
        ...
        body: JSON.stringify({ services: selectedServices, date, time, comment, phone })
      })
      ...
      <div className='relative w-full max-w-md bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden animate-fadeInScale'>
        ...
        {success ? (
          <div className='p-10 text-center'>
            ...
            <h3 className='text-xl font-bold text-gray-800 mb-2'>{t('booking.success')}</h3>
            <p className='text-gray-400 text-sm mt-1'>{selectedServices.join(', ')}</p>
            <p className='text-gray-400 text-sm'>{date} · {time}</p>
          </div>
        ) : (
          <div className='p-6 sm:p-8'>
            <h2 className='text-2xl font-bold text-gray-800 mb-1'>{t('booking.title')}</h2>

            {/* Step indicator */}
            <div className='flex gap-2 mb-6 mt-4'>
              {[1,2,3].map(s => (
                <div key={s} className={`h-1 flex-1 rounded-full transition-all duration-300 ${s <= step ? 'bg-pink-500' : 'bg-gray-200'}`}/>
              ))}
            </div>
            ...
            {step === 1 && (
              <div>
                <p className='text-sm text-gray-500 mb-1'>{t('booking.select_service')}</p>
                <p className='text-xs text-gray-400 mb-3'>Можно выбрать несколько услуг сразу (например, маникюр + педикюр)</p>
                    ...
                    return (
                      <button key={s.id} onClick={() => toggleService(s.name)}
                        ...
                        <span className={`w-4 h-4 rounded-md border flex items-center justify-center shrink-0 ${
                          checked ? 'bg-pink-500 border-pink-500 text-white' : 'border-gray-300'
                        }`}>
                          {checked && '✓'}
                        </span>
                ...
                <button disabled={!selectedServices.length} onClick={() => setStep(2)}
            ...
            {step === 2 && (
              <div className='space-y-4'>
                <div>
                  <label className='text-sm text-gray-500 block mb-1.5'>{t('booking.select_date')}</label>
                ...
                <div>
                  <label className='text-sm text-gray-500 block mb-1.5'>{t('booking.select_time')}</label>
                  {slotsLoading ? (
                ...
                <input type='text' placeholder={t('booking.comment')} value={comment}
                  onChange={e => setComment(e.target.value)}
                  className='w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-pink-400'
                />

                <div className='flex gap-2'>
                  <button onClick={() => setStep(1)} className='flex-1 py-3 border border-gray-200 rounded-xl text-sm hover:bg-gray-50 transition-colors'>
                    ← Назад
                  </button>
            ...
            {step === 3 && (
              <div>
                ...
                <div className='flex gap-2'>
                  <button onClick={() => setStep(2)} className='flex-1 py-3 border border-gray-200 rounded-xl text-sm hover:bg-gray-50'>
                    ← Назад
                  </button>
