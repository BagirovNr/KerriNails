  ...
  return (
    <>
      <header className='fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm shadow-sm'>
        ...
        <div className='hidden md:flex items-center justify-between px-6 py-2 border-b border-gray-100 text-sm text-gray-500 max-w-7xl mx-auto'>
          <div className='flex items-center gap-6'>
            <a href='tel:+79992488379' className='hover:text-pink-600 transition-colors flex items-center gap-1.5'>
        ...
        <div className='flex items-center justify-between px-4 md:px-6 py-3 max-w-7xl mx-auto'>
          ...
          {/* Logo */}
          <Link to='/home' className='text-xl font-bold tracking-tight' style={{ fontFamily: 'Georgia, serif' }}>
          ...
          <div className='flex items-center gap-2'>
            ...
            {user ? (
              <div className='flex items-center gap-2'>
                {user.role === 'admin' && (
                  <Link to='/admin' className='hidden sm:inline-flex text-xs px-3 py-1.5 bg-purple-100 text-purple-700 rounded-full font-medium hover:bg-purple-200 transition-colors'>
                    Админ
                  </Link>
                )}
                <button
                ...
                <button onClick={logout} className='hidden sm:inline-flex text-xs px-3 py-1.5 text-gray-400 hover:text-red-500 transition-colors'>
                  {t('auth.logout')}
                </button>
              </div>
            ) : (
            ...
            <button
              onClick={() => user ? setBookOpen(true) : setAuthOpen(true)}
      ...
      <div className={`fixed inset-0 z-40 md:hidden transition-all duration-300 ${menuOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        <div className={`absolute inset-0 bg-black transition-opacity duration-300 ${menuOpen ? 'opacity-50' : 'opacity-0'}`} onClick={closeMenu}/>
        <div className={`absolute left-0 top-0 bottom-0 w-72 bg-white shadow-2xl transition-transform duration-300 ${menuOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
          <div className='p-5 border-b border-gray-100 flex items-center justify-between'>
            <Link to='/home' onClick={closeMenu} className='text-xl font-bold' style={{ fontFamily: 'Georgia, serif' }}>
              <span className='text-gray-800'>Kerri</span><span className='text-pink-500'> Nails</span>
            </Link>
            <button onClick={closeMenu} className='text-gray-400 hover:text-gray-600 text-2xl leading-none'>×</button>
          </div>
          ...
          <div className='p-5 border-t border-gray-100 space-y-3'>
            <a href='tel:+79992488379' className='flex items-center gap-2 text-gray-600 text-sm'>
              📞 +7 (999) 248-83-79
            </a>
            {user ? (
              <div className='flex flex-col gap-2'>
                <span className='text-sm text-gray-500'>👤 {user.name}</span>
                <button onClick={() => { logout(); closeMenu() }} className='text-sm text-red-500 text-left'>Выйти</button>
              </div>
            ) : (
              <button onClick={() => { setAuthOpen(true); closeMenu() }}
                className='w-full py-2.5 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50 transition-colors'>
                {t('auth.login')} / {t('auth.register')}
              </button>
            )}
            <button
              onClick={() => { user ? setBookOpen(true) : setAuthOpen(true); closeMenu() }}
