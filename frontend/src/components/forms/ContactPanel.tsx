...
export default function ContactPanel() {
	...
	return (
		<>
			{/* Floating icons */}
			<div
				className={`fixed top-1/2 -translate-y-1/2 right-0 z-40 flex flex-col items-center gap-2 transition-transform duration-500 ${open ? '-translate-x-80 sm:-translate-x-96' : 'translate-x-0'}`}
			>
				{(['call', 'email'] as const).map(type => (
					<button
						key={type}
						onClick={() => setOpen(o => (o === type ? null : type))}
						className={`w-11 h-11 rounded-l-xl flex items-center justify-center shadow-md transition-all ${
							open === type
								? 'bg-pink-600 text-white'
								: 'bg-white hover:bg-pink-50 text-pink-500'
						}`}
					>
			...
			{/* Side panel */}
			<div
				className={`fixed top-0 right-0 h-full w-full sm:w-96 max-w-full bg-white shadow-2xl z-[45] transition-transform duration-500 ${open ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}
			>
				{open && (
					<>
						...
						<div className='flex-1 p-6 sm:p-8 flex flex-col'>
							<button
								...
								className='self-end mb-4 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400 text-xl transition-colors'
							>
								×
							</button>
			...
			{open && (
				<div
					className='fixed inset-0 bg-black/40 z-[44] transition-opacity'
					onClick={() => setOpen(null)}
				/>
			)}
		</>
