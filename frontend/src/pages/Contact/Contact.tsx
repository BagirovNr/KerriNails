...
export default function Contact() {
  ...
  return (
    <div className='py-12 px-4 max-w-5xl mx-auto'>
      ...
      <p className='text-gray-400 text-center mb-10'>Всегда рады вашему визиту</p>

      <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
        {/* Info cards */}
        <div className='flex flex-col gap-4'>
          {[
            { icon: '📞', label: t('contact.phone'), value: '+7 (999) 248-83-79', href: 'tel:+79992488379' },
            { icon: '📍', label: t('contact.address'), value: 'Всеволожский пр., 7, Санкт-Петербург', href: 'https://yandex.ru/maps/org/kerii_nailss/109264447499/' },
            { icon: '🕐', label: t('contact.hours'), value: t('contact.hours_value'), href: null },
          ].map(item => (
          ...
          <div className='bg-pink-50 rounded-2xl p-5'>
            <p className='text-sm font-semibold text-gray-700 mb-3'>Мы в социальных сетях</p>
            <div className='flex gap-3'>
              {[
                { name: 'ВКонтакте', href: '#', color: 'bg-blue-500' },
                { name: 'Instagram', href: '#', color: 'bg-gradient-to-br from-purple-500 to-pink-500' },
                { name: 'Telegram', href: '#', color: 'bg-sky-500' },
              ].map(s => (
                <a key={s.name} href={s.href}
                  className={`${s.color} text-white text-xs font-medium px-3 py-1.5 rounded-lg hover:opacity-90 transition-opacity`}>
                  {s.name}
                </a>
              ))}
            </div>
